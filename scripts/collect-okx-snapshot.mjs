...
function buildFlags({ stage, devRugs, top10, snipers, bundlers, securityVerdict, liquidity }) {
  const flags = [];
...
function buildRecommendedChecks(securityVerdict) {
  if (securityVerdict === "block") return ["Do not trade", "Track only for pattern learning"];
  return ["Run token-bundle-info", "Inspect top-trader addresses", "Re-scan security before any swap"];
}
...
function normalizeToken(raw, index, chainName, enrichments = {}) {
  const merged = flattenObjects(raw, enrichments.details, enrichments.dev, enrichments.bundle, enrichments.price, enrichments.advanced);
  ...
  const volume24h = numberFrom(merged, ["volume24h", "volume24H", "volume", "vol24h"], 0);
  const bondingProgress = Math.min(100, numberFrom(merged, ["bondingPercent", "bondingProgress", "progress"], 0));
  const devRugs = numberFrom(merged, ["rugPullCount", "devRugCount", "rugCount"], 0);
  const devLaunches = numberFrom(merged, ["launchCount", "devLaunchCount", "tokenCreateCount"], 0);
  const top10 = numberFrom(merged, ["top10HoldingsPercent", "top10HolderPercent", "top10HoldersPercent"], 0);
  const snipers = numberFrom(merged, ["sniperCount", "snipers", "sniperWalletCount"], 0);
  ...
  const baseRisk = devRugs * 13 + Math.max(0, top10 - 28) * 0.9 + bundlers * 5 + snipers * 0.6 + (liquidity > 0 && liquidity < 10_000 ? 18 : 0);
  ...
  return {
    ...
    ageMinutes: numberFrom(merged, ["ageMinutes", "tokenAge", "minutesSinceLaunch"], 0),
    marketCap,
    liquidity,
    volume24h,
    ...
    flags: buildFlags({ stage, devRugs, top10, snipers, bundlers, securityVerdict, liquidity }),
    opportunities: buildOpportunities({ smartMoneyScore, bondingProgress, liquidity, securityVerdict }),
    recommendedChecks: buildRecommendedChecks(securityVerdict),
